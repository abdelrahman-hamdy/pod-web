...
// Only initialize Pusher if broadcasting is enabled
let pusher = null;
if (chatify.broadcastingEnabled && chatify.pusher && chatify.pusher.key) {
    try {
        pusher = new Pusher(chatify.pusher.key, pusherConfig);
    } catch (e) {
        console.warn('Pusher initialization failed, falling back to polling:', e);
        chatify.pollingEnabled = true;
        chatify.pollingInterval = 3000;
    }
} else {
    // Use HTTP polling instead
    chatify.pollingEnabled = true;
    chatify.pollingInterval = 3000;
    console.log('Chat: Broadcasting disabled, using HTTP polling for real-time updates.');
}
/**
 *-------------------------------------------------------------
 * Re-usable methods
...
function attachmentTemplate(fileType, fileName, imgURL = null) {
  if (fileType != "image") {
    return (
      `
 <div class="attachment-preview bg-slate-50 border border-slate-200 rounded-lg p-3 mb-3 flex items-center justify-between">
     <div class="flex items-center space-x-3 flex-1 min-w-0">
         <div class="flex-shrink-0 w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center">
             <i class="ri-file-line text-indigo-600 text-lg"></i>
         </div>
         <div class="flex-1 min-w-0">
             <p class="text-sm font-medium text-slate-800 truncate">` +
      escapeHtml(fileName) +
      `</p>
             <p class="text-xs text-slate-500">File attachment</p>
         </div>
     </div>
     <button class="cancel flex-shrink-0 w-8 h-8 bg-red-100 hover:bg-red-200 text-red-600 hover:text-red-700 rounded-lg flex items-center justify-center transition-colors" title="Remove attachment">
...
`
    ...
    return (
      `
<div class="attachment-preview bg-slate-50 border border-slate-200 rounded-lg p-3 mb-3">
 <div class="flex items-center justify-between mb-2">
     <div class="flex items-center space-x-3 flex-1 min-w-0">
         <div class="flex-shrink-0 w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center">
             <i class="ri-image-line text-indigo-600 text-lg"></i>
         </div>
         <div class="flex-1 min-w-0">
             <p class="text-sm font-medium text-slate-800 truncate">` +
      escapeHtml(fileName) +
      `</p>
             <p class="text-xs text-slate-500">Image attachment</p>
         </div>
     </div>
     <button class="cancel flex-shrink-0 w-8 h-8 bg-red-100 hover:bg-red-200 text-red-600 hover:text-red-700 rounded-lg flex items-center justify-center transition-colors" title="Remove attachment">
         <i class="ri-close-line text-sm"></i>
 ...
 <div class="image-file chat-image rounded-lg overflow-hidden shadow-sm cursor-pointer hover:opacity-90 transition-opacity" 
      style="background-image: url('` +
      imgURL +
      `'); background-size: cover; background-position: center; min-height: 120px; max-width: 200px;">
 </div>
</div>
`
    );
...
// subscribe to the channel (only if Pusher is available)
const channelName = "private-chatify";
var channel = null;
var clientSendChannel = null;
var clientListenChannel = null;

if (pusher) {
  try {
    channel = pusher.subscribe(`${channelName}.${auth_id}`);
  } catch (e) {
    console.warn('Pusher subscription failed, using polling:', e);
    chatify.pollingEnabled = true;
  }
}

function initClientChannel() {
  if (pusher && getMessengerId()) {
    try {
      clientSendChannel = pusher.subscribe(`${channelName}.${getMessengerId()}`);
      clientListenChannel = pusher.subscribe(`${channelName}.${auth_id}`);
    } catch (e) {
      console.warn('Client channel subscription failed, using polling:', e);
      chatify.pollingEnabled = true;
    }
  }
}
...
if (chatify.pollingEnabled) {
  startPolling();
  console.log('HTTP polling started for real-time chat updates.');
}

// Listen to messages via Pusher (only if Pusher is available)
if (channel) {
...
$(document).ready(function () {
  ...
  // check if pusher has access to the channel [Internet status] (only if Pusher is available)
  if (pusher && pusher.connection) {
    ...
    // If no Pusher, initialize chat immediately with polling
    if (getMessengerId() != 0) {
      IDinfo(getMessengerId());
    }
  }
  ...
  // Search input on focus - MODIFIED: Don't hide chats on focus
  ...
  // Search action on keyup - MODIFIED: Only hide chats when typing
...
if (typeof EmojiButton !== 'undefined') {
  ...
  console.warn('EmojiButton library not loaded');
}
