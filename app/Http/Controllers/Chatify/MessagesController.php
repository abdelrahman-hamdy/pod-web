...
class MessagesController extends Controller
{
    ...
    public function fetch(Request $request)
    {
        ...
        // if there are no messages yet, return empty (no hint)
        if ($totalMessages < 1) {
            $response['messages'] = '';
    ...
    public function favorite(Request $request)
    {
        ...
        // Broadcast favorite update via Pusher/Reverb
        try {
            Chatify::push(
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        // send the response
        return Response::json([
            'shared' => count($shared) > 0 ? $sharedPhotos : '<p class="col-span-3 text-center text-slate-400 text-sm py-4">No shared photos yet</p>',
        ], 200);
    }
